...
from glycresoft_sqlalchemy.scoring import target_decoy, score_spectrum_matches, pair_counting
...
def test_main():
    ...
    # # frequency_counter = pair_counting.pickle.load(open(r'datafiles\Phil-82-Training-Data\pair_counts.pkl'))
    ...
    #     db_file_name, hypothesis_id, hsm_id,# scorer=scorer,
    #     n_processes=6)
    # job.start()

    # job = score_spectrum_matches.SimpleSpectrumAssignment(
    #     db_file_name, decoy_hypothesis_id, hsm_id,# scorer=scorer,
    #     n_processes=6)
    # job.start()
