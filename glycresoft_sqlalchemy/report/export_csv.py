...
def export_matches_as_csv(database_path, hypothesis_id=1, output_path=None):
    ...
    with open(output_path, 'wb') as fh:
        ...
        for gpm in gpms:
            ...
            ' '.join(map(str, gpm.scan_id_range)), gpm.glycopeptide_sequence, gpm.mean_coverage, gpm.mean_hexnac_coverage,
            len(gpm.stub_ions),
            len(gpm.bare_b_ions), 
            len(gpm.bare_y_ions),
...
def taskmain():
    ...
    if args.hypothesis_id is None:
        ...
        hypothesis_ids = [x[0] for x in dbm.session().query(Hypothesis.id).all()]
    else:
